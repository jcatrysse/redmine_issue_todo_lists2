    ...
    class ControllerIssuesEditAfterSaveHook < Redmine::Hook::ViewListener
      def controller_issues_edit_after_save(context = {})
        ...
        if (selected_todo_lists = context.dig(:params, :issue, :issue_todo_list_ids))
          allowed_todo_lists_add = IssueTodoList.where(project_id: issue.project.self_and_ancestors.ids).order('project_id', 'title').select { |list| User.current.admin? || User.current.allowed_to?(:add_issue_todo_list_items, list.project) }
          allowed_todo_lists_remove = IssueTodoList.where(project_id: issue.project.self_and_ancestors.ids).order('project_id', 'title').select { |list| User.current.admin? || User.current.allowed_to?(:remove_issue_todo_list_items, list.project) }
